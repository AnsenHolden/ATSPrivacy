...
def main():
    ...
    if compute_privacy_score:
        ...
        pri_mean, pri_var, pri_score = stat(metric_list)
        pri_time = time.time() - pri_eval_start
        print('Privacy Evaluation: (time cost: {} s)'.format(int(pri_time)))
        print("\tprivacy score: {}\tmean: {}\tvar: {}".format('%.4f' % pri_score, '%.4f' % pri_mean, '%.4f' % pri_var))
