...
def system_startup(args=None, defs=None):
    ...
    print('Currently evaluating -------------------------------:')          # 当前硬件环境
    print(datetime.datetime.now().strftime("%A, %d. %B %Y %I:%M%p"))        # 实验时间
    print(f'CPUs: {torch.get_num_threads()}, GPUs: {torch.cuda.device_count()} on {socket.gethostname()}.')     # cpu内核数量, gpu数目, 主机名
    if args is not None:
        print(args)
    if defs is not None:
        print(repr(defs))
